...
class StompManager(
    ...
    // StompManager.kt — 추가
    fun connectGlobal(userId: String,
                      onMessage: (String) -> Unit = {},
                      onError: (String) -> Unit = {}) {
        ...
        val life = stompClient!!.lifecycle()
            ...
            .subscribe({ e ->
                if (e.type == ua.naiksoftware.stomp.dto.LifecycleEvent.Type.OPENED) {
                    // 내 개인함 구독: /user/queue/inbox
                    val sub = stompClient!!.topic("/user/queue/inbox")
                        .observeOn(io.reactivex.android.schedulers.AndroidSchedulers.mainThread())
                        .subscribe({ msg -> onMessage(msg.payload) },
                            { err -> onError(err.message ?: "inbox error") })
                    bag.add(sub)
                }
                if (e.type == ua.naiksoftware.stomp.dto.LifecycleEvent.Type.ERROR) {
                    onError(e.exception?.message ?: "stomp error")
                }
            }, { err -> onError(err.message ?: "lifecycle error") })
        bag.add(life)

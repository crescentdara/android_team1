...
public class UserController {
    ...
    public ResponseEntity<Map<String, String>> updateUser(@RequestBody SignupRequestDto dto) {
        ...
        Optional<Users> optionalUser = Optional.ofNullable(usersRepository.findByUsersId(userId));
        ...
        if (optionalUser.isPresent()) {
            ...
            System.out.println("[update-user] 사용자 없음: " + userId);
    ...
    public ResponseEntity<SignupRequestDto> getUserInfo(@RequestParam String userId) {
        ...
        return Optional.ofNullable(usersRepository.findByUsersId(userId))
                .map(user -> {
    ...
    // 내부 응답 DTO 클래스 (원하면 따로 빼도 됨)
    static class CheckIdResponse {
        private boolean available;

        public CheckIdResponse(boolean available) {
